...
- **生データ編集モード**: Ctrl+Eで編集モードに切り替え、data.txtファイルを直接編集。2つの編集方法を提供：①セル編集（クリック）で素早い修正、②詳細編集ボタン（✏️）でRegisterModalを使った包括的編集（フォルダ処理・DIRオプション等も設定可能）。編集モード時はウィンドウサイズが自動拡大（1000x700px）し、フォーカスアウトでも非表示にならない
