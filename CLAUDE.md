...
### 2025-07-05: ドラッグ&ドロップ登録機能の追加
- ファイルやフォルダをドラッグ&ドロップで登録できる機能を実装
- 登録時にアイテム名、保存先タブ、引数などを設定可能
- フォルダは「フォルダ自体」または「内容を展開（dir,）」を選択可能
- `webUtils.getPathForFile()`を使用したセキュアなファイルパス取得

### 2025-07-05: データ処理の改善
- ショートカットファイル(.lnk)の処理を統一化
- コード重複の除去とリファクタリング
- データ処理システムの堅牢性向上
