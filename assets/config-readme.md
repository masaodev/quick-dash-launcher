...
- `issues[].kind` の意味:
  ...
  - `normalized`: `$schema`・`version`・`order` などの補完、存在しない参照（`groupId` 等）の除去、
    `parentGroupId` の循環解消。基本は対応不要だが、参照が除去されたときは意図した所属か確認する
    （不正な要素とそれを指す参照は消さずに残す。書き戻し時に不正な要素は配列の末尾へ移る）
