...
describe('TabStateCalculator', () => {
  describe('hasUnsavedChanges', () => {
    it('保存済み状態と現在の状態が同じ場合はfalseを返す', () => {
      const savedState: SavedTabState = {
        dataFileTabs: [{ name: 'メイン', files: ['datafiles/data.json'] }],
        dataFileLabels: { 'datafiles/data.json': 'メイン用データファイル' },
      };
      const currentTabs: DataFileTab[] = [{ name: 'メイン', files: ['datafiles/data.json'] }];
      const currentLabels = { 'datafiles/data.json': 'メイン用データファイル' };
    ...
    it('タブ名が変更されている場合はtrueを返す', () => {
      ...
      const currentTabs: DataFileTab[] = [{ name: 'メイン変更後', files: ['datafiles/data.json'] }];
      const currentLabels = {};
      const pendingOps: PendingFileOperations = { filesToCreate: [], filesToDelete: [] };
    ...
    it('ラベルが変更されている場合はtrueを返す', () => {
      ...
      const currentTabs: DataFileTab[] = [{ name: 'メイン', files: ['datafiles/data.json'] }];
      const currentLabels = { 'datafiles/data.json': 'カスタムラベル' };
      const pendingOps: PendingFileOperations = { filesToCreate: [], filesToDelete: [] };
    ...
    it('ファイル作成が保留中の場合はtrueを返す', () => {
      ...
      const pendingOps: PendingFileOperations = {
        filesToCreate: ['datafiles/data2.json'],
        filesToDelete: [],
      };
    ...
    it('ファイル削除が保留中の場合はtrueを返す', () => {
      ...
      const pendingOps: PendingFileOperations = {
        filesToCreate: [],
        filesToDelete: ['datafiles/data2.json'],
      };
    ...
    it('複数の変更が同時に発生している場合はtrueを返す', () => {
      ...
      const pendingOps: PendingFileOperations = {
        filesToCreate: ['datafiles/data2.json'],
        filesToDelete: ['datafiles/data3.json'],
      };
  ...
  describe('getFileDeletionType', () => {
    it('作成予定のファイルを削除する場合はcancelCreationを返す', () => {
      const updatedTabs: DataFileTab[] = [{ name: 'メイン', files: ['datafiles/data.json'] }];
    ...
    it('他のタブでも使用されているファイルを削除する場合はremoveFromTabを返す', () => {
      ...
      const result = TabStateCalculator.getFileDeletionType('datafiles/data.json', [], updatedTabs);
    ...
    it('作成予定かつ他のタブでも使用されている場合はcancelCreationを優先', () => {
      const updatedTabs: DataFileTab[] = [
        { name: 'メイン', files: ['datafiles/data2.json'] },
  ...
  describe('getFilesToDeleteOnTabRemoval', () => {
    it('タブに含まれるファイルが他のタブで使用されていない場合は削除リストに含める', () => {
      ...
      expect(result).toEqual(['datafiles/data2.json']);
    });
    ...
    it('作成予定のファイルは削除リストに含めない', () => {
      ...
      const result = TabStateCalculator.getFilesToDeleteOnTabRemoval(
        1,
        ['datafiles/data2.json'],
        allTabs
      );
    ...
    it('複数ファイルがあり、一部は他のタブでも使用されている場合', () => {
      const allTabs: DataFileTab[] = [
        { name: 'メイン', files: ['datafiles/data.json'] },
        { name: 'サブ1', files: ['datafiles/data.json', 'datafiles/data2.json'] },
      ];
      const result = TabStateCalculator.getFilesToDeleteOnTabRemoval(1, [], allTabs);
    ...
    it('複数ファイルがあり、すべて他のタブで使用されていない場合', () => {
      const allTabs: DataFileTab[] = [
        { name: 'メイン', files: ['datafiles/data.json'] },
        { name: 'サブ1', files: ['datafiles/data2.json', 'datafiles/data3.json'] },
      ];
      const result = TabStateCalculator.getFilesToDeleteOnTabRemoval(1, [], allTabs);
      expect(result).toEqual(['datafiles/data2.json', 'datafiles/data3.json']);
    });

    it('範囲外のタブインデックスの場合は空配列を返す', () => {
      const allTabs: DataFileTab[] = [{ name: 'メイン', files: ['datafiles/data.json'] }];
