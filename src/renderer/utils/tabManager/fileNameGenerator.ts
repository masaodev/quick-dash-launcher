...
export const FileNameGenerator = {
  /**
   * デフォルトのタブ名を生成
   * @param fileName - ファイル名（例: data.txt, data2.txt）
   * @returns デフォルトのタブ名（例: メイン、サブ1）
   *
   * @example
   * getDefaultTabName('data.txt') // => 'メイン'
   * getDefaultTabName('data2.txt') // => 'サブ1'
   * getDefaultTabName('data10.txt') // => 'サブ9'
   */
   ...
   *
   * @example
   * getDefaultFileLabel('data.txt', 'メイン') // => 'メイン用データファイル'
   * getDefaultFileLabel('data2.txt', 'サブ1') // => 'サブ1用データファイル'
   * getDefaultFileLabel('data.txt', undefined, tabs) // => タブから自動取得
   */
   ...
   * @returns 次の番号のファイル名（例: data2.txt, data3.txt）
   *
   * @example
   * getNextAvailableFileName(['data.txt'], []) // => 'data2.txt'
   * getNextAvailableFileName(['data.txt', 'data2.txt'], []) // => 'data3.txt'
   * getNextAvailableFileName(['data.txt', 'data3.txt'], []) // => 'data4.txt' (欠番は埋めない)
   * getNextAvailableFileName(['data.txt'], ['data2.txt']) // => 'data3.txt'
   */
