...
export function setupWindowHandlers(
  ...
  // 3段階ピンモード用のIPCハンドラー
  ipcMain.handle(GET_WINDOW_PIN_MODE, () => {
    return getWindowPinMode();
  });

  ipcMain.handle(CYCLE_WINDOW_PIN_MODE, () => {
  ...
  ipcMain.handle(SET_EDIT_MODE, async (_event, editMode: boolean) => {
    await setEditMode(editMode);
  });

  ipcMain.handle(GET_EDIT_MODE, () => {
    return getEditMode();
  });

  // 管理ウィンドウ関連のIPCハンドラー
  ipcMain.handle(SHOW_EDIT_WINDOW, async () => {
    await showAdminWindow();
  });

  ipcMain.handle(HIDE_EDIT_WINDOW, () => {
    hideAdminWindow();
  });

  ipcMain.handle(TOGGLE_EDIT_WINDOW, async () => {
    await toggleAdminWindow();
  });

  ipcMain.handle(IS_EDIT_WINDOW_SHOWN, () => {
  ...
  ipcMain.handle(GET_INITIAL_TAB, () => {
    return getInitialTab();
  });

  ipcMain.handle(COPY_TO_CLIPBOARD, (_event, text: string) => {
    try {
  ...
  // パフォーマンス計測のIPCハンドラー
  ipcMain.handle(LOG_PERFORMANCE_TIMING, (_event, label: string, duration: number) => {
    windowLogger.info(`[Performance] ${label}: ${duration.toFixed(2)}ms`);
  });

  // ワークスペースウィンドウ関連のIPCハンドラー
  ipcMain.handle(WORKSPACE_TOGGLE_WINDOW, async () => {
    await toggleWorkspaceWindow();
  });

  ipcMain.handle(WORKSPACE_SHOW_WINDOW, async () => {
    await showWorkspaceWindow();
  });

  ipcMain.handle(WORKSPACE_HIDE_WINDOW, () => {
    hideWorkspaceWindow();
    return true;
  });

  // ワークスペースウィンドウのピン留め関連ハンドラー
  ipcMain.handle(WORKSPACE_GET_ALWAYS_ON_TOP, () => {
    return getWorkspaceAlwaysOnTop();
  });

  ipcMain.handle(WORKSPACE_TOGGLE_ALWAYS_ON_TOP, () => {
  ...
  ipcMain.handle(WORKSPACE_SET_OPACITY, async (_event, opacityPercent: number) => {
    try {
      // 値の検証（0-100の範囲にクリップ）
  ...
  ipcMain.handle(WORKSPACE_GET_OPACITY, async () => {
    try {
      const settingsService = await SettingsService.getInstance();
  ...
  ipcMain.handle(WORKSPACE_SET_SIZE, (_event, width: number, height: number) => {
    try {
      const workspace = getWorkspaceWindow();
      if (workspace && !workspace.isDestroyed()) {
