...
describe('urlConversionRules', () => {
  describe('findMatchingRule', () => {
    ...
    it('既にプレフィックスが付いたURLでもマッチする', () => {
      const url =
        'ms-excel:ofe|ofc|u|https://company.sharepoint.com/sites/team/Documents/file.xlsx';
      const rule = findMatchingRule(url);
      expect(rule).toBeDefined();
      expect(rule?.name).toBe('SharePoint/OneDrive Excel');
    });
  ...
  describe('applyConversionPrefix', () => {
    ...
    it('通常のURLにプレフィックスを追加できる', () => {
      ...
      expect(result).toBe(
        'ms-excel:ofe|u|https://company.sharepoint.com/sites/team/Documents/file.xlsx'
      );
    });
    ...
    it('既にプレフィックスが付いたURLを変換すると、古いプレフィックスが削除される', () => {
      ...
      expect(result).toBe(
        'ms-excel:ofv|u|https://company.sharepoint.com/sites/team/Documents/file.xlsx'
      );
    });

    it('複数パイプのプレフィックスも正しく削除される', () => {
      const urlWithMultiplePrefix =
        'ms-excel:ofe|ofc|u|https://company.sharepoint.com/sites/team/Documents/file.xlsx';
    ...
    it('Wordプレフィックスも正しく処理される', () => {
      ...
      expect(result).toBe(
        'ms-word:ofe|u|https://company.sharepoint.com/sites/team/Documents/file.docx'
      );
    });
    ...
    it('PowerPointプレフィックスも正しく処理される', () => {
      ...
      expect(result).toBe(
        'ms-powerpoint:ofe|u|https://company.sharepoint.com/sites/team/Documents/file.pptx'
      );
    });
    ...
    it('http（sなし）のURLも正しく処理される', () => {
      ...
      expect(result).toBe(
        'ms-excel:ofe|u|http://company.sharepoint.com/sites/team/Documents/file.xlsx'
      );
    });
  });
