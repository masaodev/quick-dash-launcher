...
#### モーダルでの編集内容
- **単一アイテム**: 名前、パス、引数、カスタムアイコンを編集可能
- **フォルダ取込ディレクティブ**:
  - パスとフォルダ取込オプション（深さ、タイプ、フィルター等）を編集可能
  - 名前フィールドは読み取り専用（ハイフン表示）
- **グループ**: グループ名とアイテム名リストを編集可能
...
#### タブの表示内容
- **タブラベル**: カスタムタブ名（`dataFileTabNames`に設定されている場合）、デフォルトタブ名、またはファイル名
  - カスタムタブ名が設定されている場合は、その名前を表示
  - タブ名が未設定の場合は、デフォルトタブ名を表示（data.txt → 「メイン」、data2.txt → 「サブ1」）
...
#### タブの表示順序
- **カスタム順序**: `tabOrder`設定に従った順序で表示
- **デフォルト順序**: `tabOrder`が空配列の場合、ファイル名の自然順（data.txt, data2.txt, data3.txt...）
...
- **タブ名カスタマイズ**: `dataFileTabNames` - 各タブの表示名
- **タブ順序**: `tabOrder` - タブの表示順序
