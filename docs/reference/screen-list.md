...
| **EditModeView** | `src/renderer/components/EditModeView.tsx` | AdminTabContainer | 編集モードタブ選択時 | 編集モードのメインビュー、ヘッダー・ステータスバー |
| **EditableRawItemList** | `src/renderer/components/EditableRawItemList.tsx` | EditModeView | 編集モードタブ選択時 | 編集可能リスト、セル編集、整列・削除 |
...
1. **管理ウィンドウ表示** → AdminTabContainer表示（デフォルト：編集モードタブ）
