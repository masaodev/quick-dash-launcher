...
| **重要** | このオブジェクトに登録されているファイル名がタブ表示の対象となります |
...
**動作仕様:**
- **タブ表示の基準**: `dataFileTabNames`に登録されているファイル名のみがタブとして表示されます
- **物理ファイルとの関係**:
  - 登録されているファイル名の物理ファイルが存在しない場合、自動的に作成されます
  - 物理ファイルが存在しても、`dataFileTabNames`に登録されていない場合はタブに表示されません
- **キー**: データファイル名（例: `data.txt`, `data2.txt`）
- **値**: タブに表示するカスタム名（空文字列の場合はデフォルト名が表示）
...
**ファイル管理との連携**:
- データファイル追加時: 新しいファイル名がこのオブジェクトに自動的に追加されます
- データファイル削除時: 該当するファイル名がこのオブジェクトから自動的に削除されます
...
**ファイル管理との連携**:
- データファイル削除時: 該当するファイル名がこの配列から自動的に削除されます
