...
3. **実行間隔**: 各アイテムの起動間隔は100ms（`GROUP_LAUNCH_DELAY_MS`定数）です
...
| 読み込みレポート       | 読み込みのたびに `config/last-load-report.json` を書き出す。直接編集した側が「受理されたか・何がスキップされたか」を確認する用途（トーストは AI から見えないため）。`files[]` にはデータファイルに加えてワークスペースファイル（`workspace.json` / `workspace-archive.json`）も並ぶ。詳細は[ワークスペースファイル形式](workspace-format.md)を参照                                                                                                                                                                        |
