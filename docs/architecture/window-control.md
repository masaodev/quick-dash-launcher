...
v0.5.9以前の旧CSV形式のデータファイルは、v0.5.10以降のアプリケーション起動時に自動的にJSON形式に変換されます。詳細は **[データファイル形式 - ウィンドウ操作アイテム](data-format.md#データ形式の変更履歴)** を参照してください。

**JSON形式専用化の利点:**
- CSV形式で発生していたフィールド順序エラー（設定値のズレ）を完全に解消
- 管理画面でのインライン編集を禁止し、JSON文字列の破損を防止
- 詳細編集モーダル（RegisterModal）からの編集で、正しいJSON形式とCSVエスケープを保証
