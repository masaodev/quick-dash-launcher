...
ウィンドウが非表示になる際、レンダラープロセスに`window-hidden`イベントが送信されます。これにより、ウィンドウ非表示時に必要な前処理（タブのリセット等）を実行できます。
...
4. レンダラープロセスが`onWindowHidden`イベントハンドラーで前処理を実行（例: タブを元に戻す）
5. ウィンドウが非表示になる

**メリット:**
- 次回ウィンドウを表示する際、既に前処理が完了しているため、表示遅延が発生しない
- 例: タブをデフォルト位置にリセットする処理をウィンドウ非表示時に実行することで、次回表示時は既にリセット済み

**実装場所:**
- メインプロセス: `src/main/windowManager.ts` - `hideMainWindowInternal()`関数
- プリロードAPI: `src/main/preload.ts` - `onWindowHidden`イベントハンドラー
- レンダラー: `src/renderer/hooks/useDataFileTabs.ts` - タブリセット処理
