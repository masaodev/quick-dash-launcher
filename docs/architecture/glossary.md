...
| ディレクティブ | - | ディレクティブ | - | データファイル内で特殊な処理を指示する行の総称。行頭のキーワード（`dir,`/`group,`/`window,`）で識別される。フォルダ取込ディレクティブ、グループディレクティブ、ウィンドウ操作ディレクティブの3種類がある。 |
| ディレクティブ | `DirDirective` | フォルダ取込ディレクティブ | フォルダ取込 | 指定フォルダ内のアイテムを自動取込するディレクティブ。データファイルでは`dir,パス,depth=1,types=file`のように記述。`isDirDirective()`、`parseDirDirective()`で判定・解析。 |
| ディレクティブ | `GroupDirective` | グループディレクティブ | グループ | 複数アイテムをグループ化するディレクティブ。データファイルでは`group,グループ名,アイテム1,アイテム2`のように記述。`isGroupDirective()`、`parseGroupDirective()`で判定・解析。 |
| ディレクティブ | `WindowDirective` | ウィンドウ操作ディレクティブ | ウィンドウ操作 | ウィンドウ検索・制御を定義するディレクティブ。データファイルでは`window,{JSON}`形式で記述（v0.5.10以降）。`isWindowOperationDirective()`で判定。 |
| 行タイプ | `LineType` | 行タイプ | - | データファイルの行の種別を表す列挙型。`RawDataLine.type`で使用。<br>値:<br>• `directive` - ディレクティブ行（`dir,`/`group,`/`window,`で始まる特殊行、ソート時は先頭に配置）<br>• `item` - アイテム行（通常のアイテム定義行、`表示名,パス[,引数][,アイコン]`形式）<br>• `comment` - コメント行（`//`で始まる行、アイテムリストには表示されない）<br>• `empty` - 空行（空白のみまたは完全に空の行、区切りとして使用） |
