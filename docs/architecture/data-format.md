...
**注意**: アプリケーションがアイテムを保存する際、カンマを含むフィールド（引数やカスタムアイコン）には自動的にCSVエスケープが適用されます。手動でダブルクォートを追加する必要はありません。
...
| **filter** | なし | globパターン | インポートするアイテムをフィルタリングするglobパターン<br>例: `*.ps1`, `*.{doc,docx,pdf}`<br>**制約**: カンマ（`,`）は使用不可 |
| **exclude** | なし | globパターン | 除外するアイテムのglobパターン<br>例: `node_modules`, `*.{tmp,temp,bak}`<br>**制約**: カンマ（`,`）は使用不可 |
| **prefix** | なし | 任意の文字列 | インポートされるアイテムの表示名に付けるプレフィックス<br>結果: `プレフィックス: アイテム名`<br>**制約**: カンマ（`,`）は使用不可 |
| **suffix** | なし | 任意の文字列 | インポートされるアイテムの表示名に付けるサフィックス<br>結果: `アイテム名 (サフィックス)`<br>**制約**: カンマ（`,`）は使用不可 |
...
| フィールド | 位置 | 必須 | 説明 |
|------------|------|------|------|
| **グループ名** | 1 | ✓ | グループの表示名<br>**制約**: カンマ（`,`）は使用不可 |
