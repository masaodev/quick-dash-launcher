...
- **タブ名選択ドロップダウン**: ヘッダー部分に「タブ名:」というラベルで表示されるドロップダウンメニューで編集対象のタブを選択
- **利用可能なタブ**: `settings.json`の`dataFileTabs`配列に登録されているファイルのみが選択肢に表示されます
- **ドロップダウン表示形式（v1.0.0以降の変更）**:
  - **タブ名優先表示**: タブ名のみ表示（ファイル名は省略）
  - カスタムタブ名が設定されている場合: タブ名のみ表示（例: `仕事用`、`プライベート`）
  - カスタムタブ名がない場合: デフォルトタブ名を表示（例: `メイン`、`サブ1`、`サブ2`）
  - **変更前**: `タブ名 (data.txt)` のように併記表示
  - **変更後**: タブ名のみ表示
