...
#### エラー一覧
- **アイテム識別情報**: アイテム名とパス/URLを両方表示（改行区切り）
- **エラーメッセージ**: 失敗原因の詳細
...
- **逐次処理**: メモリ効率とエラー耐性を向上
- **進捗通知**: 各アイテム処理時に進捗状況をリアルタイム更新（アイテム名とパス/URLを表示）
- **エラー記録**: 各フェーズで発生したエラーを詳細に記録（アイテム識別情報とエラーメッセージ）
