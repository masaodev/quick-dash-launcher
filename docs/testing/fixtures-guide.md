...
`tests/fixtures/` ディレクトリには、テストや開発時に使用する設定ファイルのテンプレートが含まれています。これにより：
...
1. `tests/fixtures/templates/data/` に新しい `.txt` ファイルを作成
...
rm tests/fixtures/dev/minimal/settings.json

# 方法2: iconsフォルダも削除（完全リセット）
rm -rf tests/fixtures/dev/minimal/icons/
rm -rf tests/fixtures/dev/minimal/favicons/
rm tests/fixtures/dev/minimal/settings.json

# 方法3: Gitでリセット（テンプレートファイルを管理している場合）
git checkout tests/fixtures/dev/minimal/
```
...
- [tests/fixtures/dev/README.md](../../tests/fixtures/dev/README.md) - 開発用テンプレートの詳細
