...
[「完全リフレッシュ」](../screens/main-window.md)等でファビコンを強制更新する場合、キャッシュをバイパスして再ダウンロードし、キャッシュを上書きします。再ダウンロードに失敗した場合（サイト閉鎖等）は、既存キャッシュがあれば縮小し直して使い続けます。既存キャッシュが画像でないデータ（HTMLエラーページ等）だった場合は削除してアイコンなし扱いとします。

個別取得（アイテム管理画面からの「ファビコンを取得」）は常にキャッシュ優先（forceRefresh=false）で動作し、キャッシュが存在すれば再ダウンロードしません。
