...
| 種類             | 表示場所                              | 用途                                                 | 特徴                       |
| ---------------- | ------------------------------------- | ---------------------------------------------------- | -------------------------- |
| **独立トースト** | 画面下部中央（専用ウィンドウ）        | アイテム起動など、ウィンドウ閉鎖後も確認が必要な通知 | クリック貫通               |
...
- `src/main/services/overlayWindowService.ts` - オーバーレイウィンドウサービス（トーストとレイアウト進捗で1ウィンドウを共用）
- `src/main/ipc/notificationHandlers.ts` - IPCハンドラー
- `overlay.html` - オーバーレイ表示用HTML（トースト＋レイアウト進捗）

**表示時間:** 通常2.5秒、グループ起動時3秒

**ウィンドウ共用について:** トーストとレイアウト進捗ミニウィンドウは1枚のオーバーレイウィンドウをモード切替で共用する。レイアウト進捗の表示中はトーストを表示しない（進捗表示を優先）。非表示のまま5分経過するとウィンドウは破棄され、次回表示時に再作成される。

**プロセス共有について:** オーバーレイウィンドウはメインウィンドウのレンダラーから `window.open` で開かれ、メインとレンダラープロセスを共有する（`src/main/services/childWindowService.ts`）。専用プロセスを持たないため、初回表示時のプロセス起動コストがなく、メモリ消費もDOM分のみ。詳細は [システム概要](../architecture/overview.md#ウィンドウとレンダラープロセスの対応)

**ウィンドウ仕様:**

- サイズ: 400×100px
- 位置: 画面下部中央（マウスカーソルがある画面の下部中央、マージン16px）
- フレームレス・透明
- クリック貫通（マウスイベント無視）
...
| ウィンドウ操作アイテムを起動                           | 独立     | 成功   | アイテム名と🪟アイコン、「ウィンドウ操作を実行します」を表示                             |
| ウィンドウを選択してアクティブ化                       | 独立     | 成功   | ウィンドウタイトルと🔄アイコン、「ウィンドウをアクティブ化しました」を表示               |
| ウィンドウを閉じる（Ctrl+D）                           | 独立     | 成功   | ウィンドウタイトルと✖️アイコン、「ウィンドウを閉じました」を表示                         |
| ウィンドウを閉じる（Ctrl+D）                           | 独立     | エラー | ウィンドウを閉じるのに失敗しました: {エラー詳細}                                         |
...
| デスクトップに移動（Ctrl+←/→）                         | 独立     | 成功   | ウィンドウタイトルと➡️アイコン、「ウィンドウをデスクトップ {番号} に移動しました」を表示 |
| デスクトップに移動（Ctrl+←/→）                         | 独立     | エラー | ウィンドウの移動に失敗しました: {エラー詳細}                                             |
| 全デスクトップに固定（Ctrl+P）                         | 独立     | 成功   | ウィンドウタイトルと📌アイコン、「ウィンドウを全デスクトップに固定しました」を表示       |
| 全デスクトップに固定（Ctrl+P）                         | 独立     | エラー | 固定に失敗しました: {エラー詳細}                                                         |
| 固定解除（Ctrl+P）                                     | 独立     | 成功   | ウィンドウタイトルと🔓アイコン、「ウィンドウの固定を解除しました」を表示                 |
| 固定解除（Ctrl+P）                                     | 独立     | エラー | 固定解除に失敗しました: {エラー詳細}                                                     |
...
| 操作                        | トースト | 種類   | メッセージ                                                                     |
| --------------------------- | -------- | ------ | ------------------------------------------------------------------------------ |
| アイテムをD&Dで追加         | 独立     | 成功   | アイテム名と⭐アイコン、パス、「ワークスペースに追加しました」を表示           |
| アイテムをD&Dで追加（失敗） | 独立     | エラー | 「ワークスペース」と⭐アイコン、「ワークスペースへの追加に失敗しました」を表示 |
