...
| ---------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------ |
| **タブバー**     | ワークスペースタブ                     | ワークスペース名を表示するタブ（クリックで切替、ダブルクリック/右クリックで名前変更、右クリックで削除） | ワークスペース存在時（常時） | [4.17](#417-ワークスペースタブ操作)              |
| **タブバー**     | タブ追加ボタン（+）                    | 新しいワークスペースを作成                                                                              | 常時表示                     | [4.17.2](#4172-タブを追加)                       |
...
| **ヘッダー**     | アーカイブタブ（📦）                   | タブバーのアーカイブタブに切り替え                                                                      | 常時表示                     | [4.4](#44-アーカイブタブをクリック)              |
...
| 4.4   | アーカイブタブをクリック        | 📦タブクリック                                    | タブバーのアーカイブタブに切り替え                           | [詳細](#44-アーカイブタブをクリック)                      |
...
| 4.7.6 | 右クリック→サブグループを追加   | 右クリック → サブグループを追加                   | 選択したグループの子としてサブグループを作成                 | [詳細](#476-グループを右クリックしてサブグループを追加)   |
| 4.7.7 | 右クリック→テキストでコピー     | 右クリック → テキストでコピー                     | グループとアイテムをテキスト形式でクリップボードにコピー     | [詳細](#477-グループを右クリックしてテキストでコピー)     |
| 4.7.8 | 右クリック→ワークスペースを移動 | 右クリック → ワークスペースを移動 → 移動先を選択  | グループと子孫サブグループ・アイテムを別ワークスペースへ移動 | [詳細](#478-グループを右クリックしてワークスペースを移動) |
...
| 4.17  | ワークスペースタブ操作          | タブバーの各種操作                                | ワークスペースの切替・作成・名前変更・削除・並べ替え         | [詳細](#417-ワークスペースタブ操作)                       |
...
1. 確認ダイアログを表示：「グループ「{グループ名}」と{N}個のアイテムをアーカイブしますか？」（サブグループがあれば個数も表示）
2. OKの場合：
...
2. サブグループ名をデフォルト値で初期化し、編集モードに入る
3. データを保存

##### 表示条件

- 深さの上限（2階層、`MAX_GROUP_DEPTH = 2`）に達しているグループでは、このメニュー項目自体が表示されない

#### 4.7.7. グループを右クリックして「テキストでコピー」
...
1. 選択したグループとその子孫サブグループの`workspaceId`を移動先に更新
2. トップレベルグループの場合は`parentGroupId`を解除
3. 所属アイテムの`workspaceId`も合わせて更新
...
1. ツールチップを表示
2. ツールチップには以下の情報を表示：
   - パス（種別に応じて「ウィンドウ操作: タイトル」「グループ: N件」「クリップボード: プレビュー」「レイアウト: N件」等の説明。`describeWorkspaceItem()`で生成）
   - リンク先（ショートカットの場合）
   - 引数（設定されている場合）
   - メモ（設定されている場合）
   - 追加日時
...
| **▶️ 起動**                                 | アイテムを起動                               | 全アイテム                             |
| **──────────────**                          | 区切り線                                     | 全アイテム                             |
| **📋 パスをコピー**                         | アイテムのパスをクリップボードにコピー       | 通常アイテム（`type: item`）のみ       |
...
| **──────────────**                          | 区切り線                                     | 全アイテム                             |
| **🔧 編集**                                 | 編集モーダル（WorkspaceItemEditModal）を開く | 全アイテム                             |
...
- `WorkspaceItemEditModal`を開き、表示名・パス・引数・ウィンドウ設定などを詳細編集する
...
ウィンドウ上部のタブバー（`WorkspaceTabBar`）でワークスペース（タブ）を管理します。
...
3. アクティブなワークスペースIDを`localStorage`（`activeWorkspaceId`）に保存
...
3. ワークスペースの並び順（`order`）を更新してデータを保存

#### 4.17.6. アーカイブタブとの関係

- タブバー末尾の「📦」ボタンはワークスペースタブとは別枠で、アーカイブ一覧表示に切り替える（4.4を参照）
...
`group.color`（workspace.jsonの保存値）には色トークン（`primary`/`teal`等）または6桁hexが格納されます。CSS変数への解決は`src/common/groupColors.ts`の`resolveGroupColorCss()`が行い、トークンは`variables.css`の`--group-color-<token>`に変換されます（例: `primary` → `var(--group-color-primary)`）。
