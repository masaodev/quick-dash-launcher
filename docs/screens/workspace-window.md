...
| **ヘッダー** | フィルタドロップダウン | アイテムタイプでフィルタリング（すべて/URL/アプリ/ファイル/フォルダ/カスタムURI） | 常時表示（v0.6.0以降） | [4.0](#40-フィルタを変更) |
...
|----|------------|---------|------------|------|
| 4.0 | フィルタを変更 | フィルタドロップダウン選択 | アイテムタイプでフィルタリング | [詳細](#40-フィルタを変更) |
...
#### アクション
- ヘッダーのフィルタドロップダウンで表示するアイテムタイプを選択
...
2. 以下のフィルタから選択：
   ...
   - **URL**: URLアイテム（http://, https://）のみ表示
   - **アプリ**: アプリケーション（.exe）のみ表示
   - **ファイル**: ファイルアイテムのみ表示
   - **フォルダ**: フォルダアイテムのみ表示
   - **カスタムURI**: カスタムURIスキーマ（obsidian://, vscode://等）のみ表示
3. 選択したタイプに一致するアイテムのみが表示される
4. フィルタはグループ化されたアイテムと未分類アイテムの両方に適用される
5. 実行履歴セクションはフィルタの影響を受けない

#### 注意
- フィルタ条件に一致するアイテムがないグループは空として表示される
- フィルタを「すべて」に戻すと、全アイテムが再表示される
