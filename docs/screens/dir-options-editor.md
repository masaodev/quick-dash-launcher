...
- [フォルダ取込](./register-modal.md#6-フォルダ取込アイテムの詳細) - フォルダ取込機能の詳細
...
実際には`JsonDirOptions`型（`src/common/types/json-data.ts`）が使用され、すべてのフィールドがオプショナルです。

```typescript
interface JsonDirOptions {
  depth?: number;          // 階層深度（0: 現在のみ、-1: 無制限）。省略時はデフォルト0
  types?: 'file' | 'folder' | 'both';  // 取得タイプ。省略時はデフォルト'both'
...
**注意**: `depth`と`types`は必須フィールドではありません。省略された場合のデフォルト値（`depth: 0`、`types: 'both'`）は`DIR_OPTIONS_DEFAULTS`定数で定義されています。処理時には`DirOptionsForProcessing`型（`depth`と`types`が必須）に変換されます。

## 4. 画面項目一覧

| 項目名 | ラベル表記 | 入力タイプ | デフォルト | 説明 | 機能詳細 |
|--------|-----------|-----------|-----------|------|----------|
| 階層深度 | 階層深度: | ドロップダウン | 0 | サブディレクトリの探索深度 | [5.1](#51-階層深度を変更) |
| 取得タイプ | 取得タイプ: | ドロップダウン | both | 取得するアイテムの種類 | [5.2](#52-取得タイプを変更) |
| フィルター | フィルター (例: *.txt): | テキスト入力 | なし | ファイル名のフィルターパターン | [5.3](#53-フィルターを入力) |
| 除外パターン | 除外パターン (例: temp*): | テキスト入力 | なし | 除外するファイル名パターン | [5.4](#54-除外パターンを入力) |
| プレフィックス | プレフィックス (例: 仕事): | テキスト入力 | なし | アイテム名の前に付ける文字列 | [5.5](#55-プレフィックスを入力) |
| サフィックス | サフィックス (例: Dev): | テキスト入力 | なし | アイテム名の後に付ける文字列 | [5.6](#56-サフィックスを入力) |
...
フォルダ取込アイテムはJSON形式でdata.jsonに保存されます（`type: "dir"`）:
...
| UIフィールド | JSONフィールド | 型 |
|-------------|-------------|---|
| 階層深度 | `options.depth` | `number`（省略可、デフォルト: `0`） |
| 取得タイプ | `options.types` | `'file' \| 'folder' \| 'both'`（省略可、デフォルト: `'both'`） |
| フィルター | `options.filter` | `string`（省略可） |
| 除外パターン | `options.exclude` | `string`（省略可） |
| プレフィックス | `options.prefix` | `string`（省略可） |
| サフィックス | `options.suffix` | `string`（省略可） |
