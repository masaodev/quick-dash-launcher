...
| **メイン** | ランチャー起動ホットキー入力欄 | 起動ホットキーの設定（デフォルト: Alt+Space） | 常時表示 | [4.1](#41-ランチャー起動ホットキー入力欄への入力) |
| **メイン** | バリデーションエラー（起動ホットキー） | ホットキーが無効な場合のエラーメッセージ | バリデーション失敗時 | [4.1](#41-ランチャー起動ホットキー入力欄への入力) |
| **メイン** | ヒントテキスト（起動ホットキー） | 設定例や修飾キーの説明 | 常時表示 | - |
| **メイン** | ウィンドウ検索で起動ホットキー入力欄 | ウィンドウ検索モード直接起動のホットキー（デフォルト: 未設定） | 常時表示 | [4.6](#46-ウィンドウ検索で起動のホットキー入力欄への入力) |
| **メイン** | バリデーションエラー（検索ホットキー） | ホットキーが無効な場合のエラーメッセージ | バリデーション失敗時 | [4.6](#46-ウィンドウ検索で起動のホットキー入力欄への入力) |
| **メイン** | ヒントテキスト（検索ホットキー） | 検索ホットキーの説明 | 常時表示 | - |
...
|----|------------|---------|------------|------|
| 4.1 | ランチャー起動ホットキー入力欄への入力 | キー押下 | ホットキーのリアルタイムバリデーション | [詳細](#41-ランチャー起動ホットキー入力欄への入力) |
...
| 4.6 | ウィンドウ検索で起動のホットキー入力欄への入力 | キー押下 | ホットキーのリアルタイムバリデーション（空許可） | [詳細](#46-ウィンドウ検索で起動のホットキー入力欄への入力) |

## 5. 機能詳細

### 4.1. ランチャー起動ホットキー入力欄への入力
...
- 起動ホットキー（`hotkeyValidation.isValid`）とウィンドウ検索ホットキー（`itemSearchHotkeyValidation.isValid`）の両方が有効な場合のみボタンが有効化される
...
### 4.6. ウィンドウ検索で起動のホットキー入力欄への入力

#### アクション
- ウィンドウ検索で起動のホットキー入力欄をクリックし、キー組み合わせを入力（またはクリアボタンで削除）

#### 処理フロー
1. ホットキー入力コンポーネントでキー入力を受け付け（`allowEmpty={true}`、`showClearButton={true}`）
...
#### 特徴
- 空文字を許可（`allowEmpty={true}`）: 設定しなくてもよい
- クリアボタン表示（`showClearButton={true}`）: 入力をリセット可能
- 設定すると、このホットキーでウィンドウ検索モードが直接開く
- 初回設定後、設定画面からいつでも変更可能

#### 設定の反映
- 完了ボタン押下時に`settings.json`の`itemSearchHotkey`に保存
- 空文字列の場合は機能が無効
...
- **背景色**: メインウィンドウと同一
- **フォント**: システムフォント
- **ボタン配置**: 下部右寄せ（`justify-content: flex-end`）に「設定を完了」ボタン1つ
...
- **キー**:
  - `hotkey`: ランチャー起動ホットキー（デフォルト: `Alt+Space`）
  - `itemSearchHotkey`: ウィンドウ検索で起動のホットキー（デフォルト: `''`、空=無効）
