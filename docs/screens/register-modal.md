...
アイテム登録・編集モーダル（RegisterModal）は、ランチャーに新しいアイテムを追加したり、既存アイテムを編集するためのモーダルダイアログです。単一アイテム、フォルダ取込、グループ、ウィンドウ操作、クリップボードの5種類（`itemCategory`）を扱うことができます。
...
- **種別切り替え**: 単一アイテム、フォルダ取込、グループ、ウィンドウ操作、クリップボードの5種類を選択
...
| **フォーム** | 種別ドロップダウン | 📄単一アイテム / 🗂️フォルダ取込 / 📦グループ / 🪟ウィンドウ操作 / 📋クリップボード | 常時表示 | [4.1](#41-種別を変更) |
| **フォーム** | 名前入力欄 | アイテムの表示名を入力 | 種別が単一アイテム、グループ、ウィンドウ操作、またはクリップボードの場合 | [4.2](#42-名前を入力) |
| **フォーム** | パス入力欄 | ファイルパス、URL、カスタムURIを入力 | 種別が単一アイテムまたはフォルダ取込の場合 | [4.3](#43-パスを入力) |
| **フォーム** | 🎨 アイコン取得ボタン | URLのファビコンを即座に取得 | アイテムタイプがURLの場合（v0.6.0以降） | [4.3.1](#431-アイコン取得ボタンをクリック) |
| **フォーム** | 引数入力欄 | コマンドライン引数を入力 | 種別が単一アイテムの場合 | [4.4](#44-引数を入力) |
| **通知** | トースト通知 | アイコン取得失敗時のエラーメッセージ | アイコン取得失敗時（v0.6.0以降） | [4.3.2](#432-トースト通知) |
...
| **フッター** | ⚡ 試しに実行ボタン | 保存せずにアイテムを実行 | グループ・フォルダ取込・クリップボード以外（単一アイテム・ウィンドウ操作のみ） | [4.15](#415-試しに実行ボタンをクリック) |
...
| 4.3.1 | アイコン取得ボタンをクリック | ボタンクリック | URLのファビコンを取得してカスタムアイコンとして設定 | [詳細](#431-アイコン取得ボタンをクリック) |
| 4.3.2 | トースト通知 | アイコン取得失敗時 | エラーメッセージを画面内に表示 | [詳細](#432-トースト通知) |
...
| 4.7 | ウィンドウ操作設定を入力 | テキスト/数値入力、ボタンクリック | ウィンドウ操作設定の更新 | [詳細](#47-ウィンドウ操作設定を入力) |
| 4.8 | グループアイテムを管理 | 削除ボタンクリック | グループメンバーの削除 | [詳細](#48-グループアイテムを管理) |
| 4.9 | グループアイテムを追加 | 追加ボタンクリック | 選択モーダルを開く | [詳細](#49-グループアイテムを追加) |
| 4.10 | 保存先タブを変更 | ドロップダウン変更 | 保存先タブの更新 | [詳細](#410-保存先タブを変更) |
| 4.11 | 保存先ファイルを変更 | ドロップダウン変更 | 保存先ファイルの更新 | [詳細](#411-保存先ファイルを変更) |
| 4.12 | カスタムアイコンを設定 | ファイル選択/削除 | カスタムアイコンの管理 | [詳細](#412-カスタムアイコンを設定) |
| 4.13 | キャンセルボタンをクリック | ボタンクリック | モーダルを閉じる | [詳細](#413-キャンセルボタンをクリック) |
| 4.14 | 登録/更新ボタンをクリック | ボタンクリック | バリデーションと登録/更新処理 | [詳細](#414-登録更新ボタンをクリック) |
...
#### アクション
- 種別ドロップダウンで「📄 単一アイテム」「🗂️ フォルダ取込」「📦 グループ」「🪟 ウィンドウ操作」「📋 クリップボード」のいずれかを選択
...
| 項目 | 単一アイテム | フォルダ取込 | グループ | ウィンドウ操作 | クリップボード |
|------|:-----------:|:-----------:|:--------:|:--------------:|:--------------:|
| 名前 | ○ | - | ○ | ○ | ○ |
| パス | ○ | ○ | - | - | - |
| 引数 | ○ | - | - | - | - |
...
| `*.exe`, `*.bat`, `*.cmd`, `*.com`, `*.lnk` ファイル | `app` |
| `obsidian://` などカスタムURI（http/https以外の `://` スキーマ） | `customUri` |

> `LauncherItem.type` の実際の値: `'url' | 'file' | 'folder' | 'app' | 'customUri' | 'clipboard'`
...
### 4.3.1. アイコン取得ボタンをクリック

#### アクション
- パス入力欄の横にある「🎨 アイコン取得」ボタンをクリック

#### 表示条件
- アイテムタイプが`url`（URLアイテム）の場合のみ表示（v0.6.0以降）

#### 処理フロー
1. パス入力欄のURLを取得
2. URLが有効か検証（http://, https:// で始まるか確認）
3. ファビコン取得APIを呼び出し
4. 取得成功時：
   - アイコンプレビューを更新
   - カスタムアイコンとして自動保存
...
#### 注意
- URLが入力されていない場合、エラー通知が表示されます
- 既存のカスタムアイコンがある場合、上書きされます
- ファビコンが存在しないURLの場合、取得に失敗します
...
#### 表示内容
- エラーメッセージ（例: 「アイコンの取得に失敗しました」）
- 自動的に3秒後に消える

#### 実装
- `useRegisterForm`フックで管理
- モーダル内に埋め込まれた通知UI
- 画面内トースト通知として表示（モーダルをブロックしない）
...
#### 注意事項
- グループアイテム・フォルダ取込アイテム・クリップボードアイテムは直接実行できないため、ボタンは表示されない（`NON_EXECUTABLE_CATEGORIES = ['dir', 'group', 'clipboard']`）
...
| `UrlConverterMenu` | URLの変換オプションメニューを表示 | 種別が単一アイテムでURLタイプの場合 |
| `IconFetchButton` | URLのファビコン取得ボタン | 種別が単一アイテムでURLタイプの場合 |
| `ClipboardItemEditor` | クリップボードアイテムのキャプチャ・プレビュー | 種別がクリップボードの場合 |
...
- `get-all-windows-all-desktops`: すべての仮想デスクトップのウィンドウ情報を取得（`window.electronAPI.getAllWindowsAllDesktops()`）

**取得される情報:**
```typescript
// src/common/types/window.ts
interface WindowInfo {
  ...
  windowState?: WindowState;    // ウィンドウ状態（'normal' | 'minimized' | 'maximized'）
  icon?: string;                // アイコン（base64エンコードされたデータURL）
  desktopNumber?: number;       // 仮想デスクトップ番号（1から開始）
...
- **実装場所**:
  ...
  - `src/main/utils/virtualDesktop/` ディレクトリ（`index.ts`, `windowOperations.ts`, `dllLoader.ts`, `registryAccess.ts`, `types.ts`, `guidUtils.ts`）
