...
1. **位置・サイズ調整:**
   | 項目 | 説明 | プレースホルダー |
   |------|------|-----------------|
   | **X座標** | ウィンドウのX座標（仮想スクリーン座標系） | 「X座標」 |
   | **Y座標** | ウィンドウのY座標（仮想スクリーン座標系） | 「Y座標」 |
   | **幅** | ウィンドウの幅（ピクセル） | 「幅」 |
   | **高さ** | ウィンドウの高さ（ピクセル） | 「高さ」 |
...
ウィンドウ操作アイテムは内部的にJSON形式でデータを保存します。管理画面（EditableRawItemList）で直接データを編集する場合、JSON形式の検証とエラー表示が行われます：

**検証内容:**
- JSON形式が正しく記述されているか（パースエラーの検出）
- `window,`の後に`{`で始まるJSON文字列が存在するか

**エラー表示:**
- JSON形式が不正な場合、該当行の名前セルに「(JSON形式エラー)」と表示されます
- エラー状態のセルは赤色で表示され、ホバー時に「JSON形式エラー: 詳細編集で修正してください」のツールチップが表示されます
- パス・引数セルにも「(JSON形式エラー)」と表示されます

**修正方法:**
1. エラー行の「✏️」ボタンをクリックして詳細編集モーダルを開く
2. RegisterModalでウィンドウ操作設定を再入力
3. 保存すると自動的に正しいJSON形式で保存されます

詳細は **[ウィンドウ制御システム](../architecture/window-control.md)** を参照してください。
