...
| タイプ | 呼び出すAPI | 対象ファイル |
|--------|------------|-------------|
| `html` | `window.electronAPI.selectBookmarkFile()` | HTMLファイル (*.html, *.htm) |
| `image` | `window.electronAPI.selectCustomIconFile()` | 画像ファイル (*.png, *.jpg, *.ico等) |
| `all` | 現在未実装 | - |
...
1. 「ファイルを参照...」ボタンをクリック
2. `fileTypes`に応じたElectron APIを呼び出し
3. OSネイティブのファイル選択ダイアログが表示
4. ユーザーがファイルを選択
5. `onFileSelect(filePath)`コールバックを実行
6. ダイアログを閉じる（`onClose()`）
