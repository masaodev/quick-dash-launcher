...
3. 複数ファイルの場合はパス配列を作成
4. `RegisterModal`を開いて登録フローへ（詳細は[アイテム登録・編集モーダル仕様書](./register-modal.md)を参照）
...
2. フォルダ取込から展開されたアイテム（`isDirExpanded: true`）の場合:
   ...
   - 元の行番号とデータファイル情報を保持
3. `RegisterModal`を編集モードで開く（詳細は[アイテム登録・編集モーダル仕様書](./register-modal.md)を参照）
...
2. `RegisterModal`を新規登録モードで開く（詳細は[アイテム登録・編集モーダル仕様書](./register-modal.md)を参照）
