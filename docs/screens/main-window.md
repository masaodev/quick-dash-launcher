...
- **進捗表示**: アイコン処理中の進捗バー表示
- **詳細情報表示**: アイテムにマウスホバーでパス、データファイル、行番号、フォルダ取込情報をツールチップ表示
...
| **メイン** | 詳細ツールチップ | アイテムホバー時にパス、データファイル、行番号、フォルダ取込情報を表示 | ホバー時 | [4.20](#420-アイテムにマウスホバー) |
...
1. 入力値の変更を検知
2. ウィンドウ検索モードに自動切り替え
3. システム内の開いているウィンドウを取得（QuickDashLauncher自身は除外）
4. `<` を除いた検索クエリでウィンドウタイトルをフィルタリング（スペース区切りでAND検索）
5. フィルタリングされたウィンドウリストを表示
6. ウィンドウアイテムには青い左境界線と 🪟 アイコンを表示

#### モード切り替え
- 検索欄の最初の文字が `<` → ウィンドウ検索モード
- `<` を削除 → 通常モードに戻る
...
- **パス情報**（最初に表示）
  ...
  - 引数あり: パスと引数を結合した完全なコマンドライン
- **空行**
- **データファイル**: アイテムが定義されているファイル（data.txt または data2.txt）
- **行番号**: データファイル内の行番号
...
- **ツールチップ**: タブにホバーすると、ファイル情報が表示される
  - **ファイルラベルが設定されている場合**: 「ラベル名 (ファイル名)」形式（例: 「開発用 (data2.txt)」）
  - **ラベルが未設定の場合**: ファイル名のみ（例: 「data.txt」）
