...
メインウィンドウは、QuickDashLauncherの中核となる検索・ランチャー画面です。起動ホットキー（Alt+Space）で呼び出され、アプリケーション、ファイル、フォルダ、URLに素早くアクセスできます。
...
- **ドラッグ&ドロップ登録**: ファイル、フォルダ、URLをドロップして簡単登録
...
| **コンポーネント名** | `App` |
| **表示条件** | 起動ホットキー（Alt+Space、デフォルト）押下時 |
| **画面タイプ** | メインウィンドウ |
| **親画面** | なし（ルートコンポーネント） |
...
| **ヘッダー** | アイテム登録ボタン（➕） | アイテム登録・インポートメニュー（ドロップダウン） | 常時表示 | [4.22](#422-アイテム登録ボタン押下) |
| **ヘッダー** | └ 📄 簡易登録 | アイテム登録モーダルを開く | ドロップダウン展開時 | [4.22](#422-アイテム登録ボタン押下) |
| **ヘッダー** | └ 🔖 ブラウザのブックマークを追加 | 管理ウィンドウのブックマークインポートを開く | ドロップダウン展開時 | [4.22](#422-アイテム登録ボタン押下) |
| **ヘッダー** | └ 💻 インストール済みアプリを追加 | 管理ウィンドウのアプリインポートを開く | ドロップダウン展開時 | [4.22](#422-アイテム登録ボタン押下) |
...
| **ヘッダー** | └ 🔖 ブックマーク取込 | 設定済みのブックマーク取込ルールを一括実行 | ドロップダウン展開時 | [4.3](#43-更新メニュー操作) |
| **ヘッダー** | └ ──────────── | 区切り線 | ドロップダウン展開時 | - |
| **ヘッダー** | └ 🔄 完全リフレッシュ | ブックマーク取込＋データ再読込＋全アイコン強制取得 | ドロップダウン展開時 | [4.3](#43-更新メニュー操作) |
...
| **ヘッダー** | └ 🗂️ ワークスペースを表示 | ワークスペースウィンドウの表示/非表示を切り替え | ドロップダウン展開時 | [4.5](#45-設定メニュー操作) |
| **ヘッダー** | └ ──────────── | 区切り線 | ドロップダウン展開時 | - |
...
| 4.5 | 設定メニュー操作 | 設定ボタン（⚙）クリック / メニュー選択 / Ctrl+E | 基本設定、アイテム管理、ワークスペース表示、終了 | [詳細](#45-設定メニュー操作) |
...
| 4.18 | ドラッグ&ドロップ操作 | ドラッグ / ドロップ操作 | ファイル・フォルダ・URLの登録 | [詳細](#418-ドラッグドロップ操作) |
| 4.19 | ウィンドウ表示時の初期化 | 起動ホットキー（デフォルト: Alt+Space） | データ読み込み、アイコン適用、画面初期化 | [詳細](#419-ウィンドウ表示時の初期化) |
...
| 4.22 | アイテム登録ボタン押下 | 登録ボタン（➕）クリック / メニュー選択 | 登録方法を選択（簡易登録・ブックマーク追加・アプリ追加） | [詳細](#422-アイテム登録ボタン押下) |
...
| 4.27 | 検索モード切り替え | Shift+Tabキー押下 | 検索モードを切り替え（通常⇔ウィンドウ） | [詳細](#427-検索モード切り替え) |
...
- 検索モード切り替えの詳細は[4.27](#427-検索モード切り替え)を参照（通常⇔ウィンドウの2モード）
...
**🔖 ブックマーク取込:**
1. 管理ウィンドウの設定で登録済みのブックマーク取込ルールをすべて実行
2. 取込結果をトースト通知で表示
3. データを再読込して画面に反映
4. 有効なブックマーク取込ルールがない場合は警告を表示

**🔄 完全リフレッシュ:**
1. ブックマーク取込ルールをすべて実行
2. データファイルを再読み込み
3. すべてのアイテムのアイコンを強制再取得（既存キャッシュを無視）
4. 進捗バーで処理状況を表示
5. 完了後、メニューを閉じる
6. 詳細ボタンまたは進捗バーをクリックで[アイコン取得進捗詳細モーダル](./icon-progress-detail-modal.md)を表示
...
- **ブックマーク取込**: 設定済みの取込ルールを一括実行（データ再読込を含む）
- **完全リフレッシュ**: ブックマーク取込＋データ再読込＋すべてのアイコンを強制再取得（最も包括的）
...
#### アクション
- 起動ホットキー（Alt+Space、デフォルト）でウィンドウを表示
...
#### アクション
- ヘッダーのアイテム登録ボタン（➕）をクリックしてドロップダウンメニューを開き、登録方法を選択
...
**🔖 ブラウザのブックマークを追加:**
1. 管理ウィンドウをブックマークインポートモーダルを開いた状態で起動
2. ブラウザのブックマークからアイテムを選択して登録

**💻 インストール済みアプリを追加:**
1. 管理ウィンドウをアプリインポートモーダルを開いた状態で起動
2. インストール済みアプリのリストからアイテムを選択して登録
...
#### ボタン配置
- ヘッダーの一番左（更新ボタンの前）、コンポーネント: `LauncherRegisterDropdown`
...
- 設定画面で「ウィンドウ検索で起動」のホットキーを設定している場合、そのホットキーで直接起動可能
...
**Shift+Tabキー押下時:**
1. 現在の検索モードを確認
2. モードをトグル切り替え（normal → window または window → normal）
3. 各モードに応じた初期化処理を実行：
   - **通常モードへ**: データファイルタブのアイテムリストを表示
   - **ウィンドウ検索モードへ**: システム内の開いているウィンドウリストを取得、仮想デスクトップ情報を取得
4. 検索クエリをクリア
5. 選択インデックスを先頭にリセット
6. モードインジケーターを更新（検索欄下部に常時表示）
