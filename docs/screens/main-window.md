...
|--------|--------|------------|----------|----------|
| **タブバー** | データファイルタブ | データファイル切り替え用のタブボタン | タブ表示ON時 | [4.23](#423-データファイルタブをクリック) |
...
| **メイン** | 詳細ツールチップ | アイテムホバー時にパス、データ元、行番号、フォルダ取込情報を表示 | ホバー時 | [4.20](#420-アイテムにマウスホバー) |
...
#### 処理フロー
1. `window.electronAPI.cycleWindowPinMode()`を実行
...
#### 処理フロー
1. 選択中のアイテムの `originalPath` プロパティを確認
2. `originalPath` が存在しない場合はエラーメッセージを表示
3. `window.electronAPI.copyToClipboard` でリンク先パスをクリップボードにコピー
4. 成功通知を表示
5. メニューを閉じる
...
#### エラーハンドリング
- `originalPath` が存在しない場合: 「ショートカットのパスが見つかりません」と表示
- コピー失敗時: アラート表示
...
1. 選択中のアイテムの `originalPath` プロパティを確認
2. `originalPath` が存在しない場合はエラーメッセージを表示
3. リンク先パスから最後のスラッシュ/バックスラッシュを検索
4. 親ディレクトリパスを抽出
5. `window.electronAPI.copyToClipboard` でクリップボードにコピー
6. 成功通知を表示
7. メニューを閉じる
...
#### エラーハンドリング
- `originalPath` が存在しない場合: 「ショートカットのパスが見つかりません」と表示
- 親フォルダーパスの取得失敗時: 「ショートカットの親フォルダーのパスを取得できませんでした」と表示
- コピー失敗時: アラート表示
...
1. 選択中のアイテムの `originalPath` プロパティを確認
2. `originalPath` が存在しない場合はエラーメッセージを表示
3. リンク先パスを使って一時的な `LauncherItem` オブジェクトを作成
4. `window.electronAPI.openParentFolder(tempItem)` を実行
5. `shell.showItemInFolder()` でエクスプローラーを起動
6. リンク先の親フォルダーを開き、リンク先ファイル/フォルダーを選択状態で表示
7. メニューを閉じる
8. ウィンドウを非表示（固定されていない場合）
...
**リンク先のパスをコピー:**
- `item.originalPath`（.lnkファイルのリンク先パス）
...
4. アイテムリストを表示
5. 検索入力欄に自動フォーカス
6. `window.electronAPI.getWindowPinMode()`で固定モードを取得
7. 固定ボタンの状態を反映
...
- **保存先が変更された場合**: 元のタブから削除し、新しいタブに登録（タブ間移動）
  - `deleteItems` APIで元のタブから削除
  - `registerItems` APIで新しいタブに登録
...
3. 上記の条件に該当しない場合：
   - `window.electronAPI.hideMainWindow()`を実行
   - メインウィンドウを非表示にする
...
**編集モード:**
- アイテム登録モーダル（RegisterModal）が開いている
- アイテム編集モーダルが開いている
- ブックマークインポートモーダルが開いている
- 誤操作でウィンドウが閉じるのを防止

**モーダルモード:**
- AlertDialog（通知ダイアログ）が表示されている
- ConfirmDialog（確認ダイアログ）が表示されている
