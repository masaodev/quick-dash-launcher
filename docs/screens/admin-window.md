...
- **関連ファイル一覧**: タブに含まれるファイルのリストを表示
  - **ファイル名**: 各ファイルの物理ファイル名（例: data.txt, data2.txt）
  - **ラベル入力欄**: 各ファイルに任意のラベル（別名）を設定可能
    - プレースホルダー: 「ラベル（任意）」
    - 入力後、フォーカスが外れると自動保存
    - ラベルを空にすると削除される
    - ラベルはグローバルで、全てのタブで共通
...
**ファイルラベルの表示:**
- タブバー: ラベルが設定されている場合、ツールチップに「ラベル名 (ファイル名)」と表示
- 編集モード: ファイル選択ドロップダウンでラベルが優先表示され、ツールチップに物理ファイル名が表示
...
- **選択肢**: 現在のタブに属する全てのファイル
  - **ファイルラベルが設定されている場合**: ラベル名を表示（例: 「開発用」）
  - **ラベルが未設定の場合**: ファイル名を表示（例: 「data2.txt」）
