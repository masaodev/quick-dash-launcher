...
| **ヘッダー** | タブボタン（📖 ヘルプ） | ヘルプタブへの切り替え | 常時表示 | [4.1](#41-タブボタンをクリック) |
| **基本設定** | ホットキー入力欄 | ランチャー起動ホットキーの設定 | 基本設定タブ | [4.2](#42-ホットキー入力欄でキー押下) |
| **基本設定** | ウィンドウ検索の起動ホットキー入力欄 | ウィンドウ検索で起動のホットキー設定 | 基本設定タブ | [4.2](#42-ホットキー入力欄でキー押下) |
...
| **アイテム管理** | ➕ アイテムを追加ボタン | 新規アイテムを先頭に追加 | アイテム管理タブ | [4.13](#413-アイテムを追加ボタンをクリック) |
| **アイテム管理** | 🗑️ 選択したアイテムを削除ボタン | 選択中のアイテムを削除 | アイテム管理タブ、選択時 | [4.14](#414-選択したアイテムを削除ボタンをクリック) |
| **アイテム管理** | 🧹 重複を削除ボタン | 現在のデータファイル内の重複アイテムを削除 | アイテム管理タブ | [4.15](#415-重複を削除ボタンをクリック) |
| **アイテム管理** | 変更を破棄ボタン | 未保存の変更（追加・削除・編集）をすべて捨てる | アイテム管理タブ、変更あり時 | [4.12](#412-変更を保存ボタンをクリック) |
...
| **アイテム管理** | ステータスバー | 選択件数、合計件数、入力不備の件数、未保存の変更件数の表示 | アイテム管理タブ | - |
| **ヘルプ** | 📁 設定フォルダを開くボタン | 設定フォルダをエクスプローラーで開く | ヘルプタブ | [4.19](#419-設定フォルダを開くボタンをクリック) |
| **ヘルプ** | アプリケーション情報 | バージョン、ライセンス情報を表示 | ヘルプタブ | - |
| **ヘルプ** | 🔗 GitHubリポジトリボタン | GitHubリポジトリを開く | ヘルプタブ | [4.22](#422-githubリポジトリボタンをクリック) |
| **ヘルプ** | ショートカット一覧 | 利用可能なキーボードショートカット | ヘルプタブ | - |
| **ヘルプ** | 🚪 アプリケーションを終了ボタン | アプリケーションを終了 | ヘルプタブ | [4.21](#421-アプリケーションを終了ボタンをクリック) |

## 4. アクション一覧

| No | アクション名 | トリガー | 主な処理内容 | 詳細 |
|----|------------|---------|------------|------|
| 4.1 | タブボタンをクリック | タブボタンクリック | タブ切り替え（アイテム管理の未保存の変更は保持される） | [詳細](#41-タブボタンをクリック) |
...
| 4.13 | アイテムを追加ボタンをクリック | ボタンクリック | 新規アイテムを先頭に追加 | [詳細](#413-アイテムを追加ボタンをクリック) |
| 4.14 | 選択したアイテムを削除ボタンをクリック | ボタンクリック / Deleteキー | 確認後、選択中のアイテムを削除 | [詳細](#414-選択したアイテムを削除ボタンをクリック) |
| 4.15 | 重複を削除ボタンをクリック | ボタンクリック | 件数を確認してから重複を削除 | [詳細](#415-重複を削除ボタンをクリック) |
...
| 4.28 | ワークスペースウィンドウ設定を変更 | スライダー・チェックボックス | ワークスペースウィンドウの透過度と背景透過設定の変更（即座反映） | [詳細](#428-ワークスペースウィンドウ設定を変更) |
| 4.29 | 右クリックメニューを表示 | テーブル行を右クリック | 行の複製・詳細編集・削除のメニューを表示 | [詳細](#429-右クリックメニューを表示) |
...
- **backupRetention**: バックアップ保存件数（3-50件、デフォルト: 10）
- **backupIncludeClipboard**: クリップボードデータもバックアップに含める
...
- **外部変更との競合**: 「データファイルが QDL の外で変更されていた（または他の画面で編集された）ため…」の
  警告を出し、最新の内容を読み込み直す。**未保存の変更は捨てず、新しい内容の上に載せ直す**（4.16 の「外部変更の取り込み」）。
  内容を確認してもう一度保存する
...
### 4.13. アイテムを追加ボタンをクリック

#### アクション
- 「➕ アイテムを追加」ボタンをクリック

#### 処理フロー
1. 単一アイテム（名前・パスが空）を作る。id はこの時点で採番する
2. 現在選択中のデータファイルの先頭に追加し、行番号を振り直す
3. 未保存の変更として扱う（名前列・パス列をクリックして入力する。種類を変えるには ✏️ 詳細編集）

### 4.14. 選択したアイテムを削除ボタンをクリック
...
2. 確認ダイアログを表示：1 件なら「「{名前}」を削除しますか？」、複数なら「{N} 件のアイテムを削除しますか？」
...
| **#** | ファイル内の位置（1 始まり）。並べ替え中は順番どおりに並ばない |
| **種類** | アイテムタイプ（単一アイテム、フォルダ取込、グループ、ウィンドウ操作、クリップボード、ウィンドウレイアウト） |
| **アイコン** | メインウィンドウと同じアイコン表示（単一アイテムのみ） |
| **名前** | 表示名（クリックして編集可能。フォルダ取込は名前を持たないため「-」） |
| **パスと引数** | パスと引数の組み合わせ（単一アイテム・フォルダ取込はパスをクリックして編集可能。グループ・ウィンドウ操作・クリップボード・レイアウトは ✏️ の詳細編集から。ダブルクリックでも開く） |
...
#### 処理フロー
1. セルの内容を編集し、Enter またはフォーカス移動で確定する
...
- 自動取込で登録されたアイテムの紐づけ（`autoImportRuleId`）は詳細編集後も残る
...
2. 選択中のファイルがそのタブに無ければ、タブの最初のファイル（files[0]）を選択する
...
- ラジオボタンで以下の2モードから選択
- **ディスプレイの端に配置（displayLeft / displayRight）**: 選択したディスプレイの左端または右端にワークスペースを固定配置（デフォルト: displayRight）
  - ディスプレイ選択ドロップダウンで対象ディスプレイを指定（workspaceTargetDisplayIndex）
  - 左端/右端の切り替えはラジオボタンで選択
- **固定位置（fixed）**: ワークスペースを移動した位置を記憶して、次回も同じ位置に表示
- 後方互換性のため、旧設定値 `primaryLeft` / `primaryRight` は `displayLeft` / `displayRight` と同等に扱われる
...
- **更新API**:
  ...
  - 保存先が同じ場合: `update-item-by-id` IPCチャンネルでアイテムを更新
...
**自動反映される操作:**
- アイテムの更新（`update-item-by-id`）
- アイテムの削除（`delete-items-by-id`）
- 一括更新（`batch-update-items-by-id`）
...
- **未保存の変更の保持**: タブまたはファイルを切り替えても未保存の変更は失われず、保存時にまとめて書き込まれます
...
#### アイテムの追加
- **アイテムを追加**: ➕アイテムを追加ボタンで、名前・パスが空の単一アイテムを先頭に作成
- 名前列・パス列をクリックして入力する。種類を変える（フォルダ取込・グループなど）には ✏️ 詳細編集を使う
- パスが未設定の間は「(パスなし)」と表示され、ステータスバーの「入力不備」に数えられる

#### セル編集
- セルをクリックして即座に編集
- **編集確定**: Enter または フォーカス移動
- **編集制限**:
  - パス列をセルで編集できるのは単一アイテムとフォルダ取込のみ
  - グループ・ウィンドウ操作・クリップボード・ウィンドウレイアウトのパス列は読み取り専用（ツールチップで案内。ダブルクリックで詳細編集が開く）
  - 編集は✏️ボタンから詳細編集モーダル（RegisterModal）を使用する
...
管理画面の「🧹 重複を削除」ボタンで、現在選択中のデータファイル内の重複アイテムを削除できます。

- **対象範囲**: 現在選択中のデータファイルのみ
- **重複判定**: 種類と表示テキスト（名前・パス・引数など）が同じアイテム。ファイル内で先にある 1 件を残す
- **確認**: 件数を示す確認ダイアログの後に削除する。重複が無ければトーストで知らせる
- **保存**: 削除後は未保存状態となり、保存ボタンで実際のファイルに反映。保存前なら「変更を破棄」で戻せる
- 整列（並べ替えて保存する機能）は廃止した。メイン画面は常に表示名の昇順で表示するため、ファイル内の並びを変えても見た目は変わらない

#### その他の操作
- **名前編集**: 単一アイテムとグループの名前列をクリック（フォルダ取込アイテムは編集不可）
- **削除**: 選択後にDeleteキーまたは削除ボタン（いずれも確認ダイアログあり。入力欄の中の Delete キーは文字の削除）
- **行複製**: 右クリックメニューから「複製」を選択（複数行の一括複製にも対応）
- **一括操作**: チェックボックスによる複数選択
- **保存**: Ctrl+S または保存ボタン（保存が成功するまで編集状態は残る）
- **破棄**: 変更を破棄ボタンで、追加・削除・編集をすべてファイルの内容に戻す
...
| `hotkey` | `string` | `''` | 起動ホットキー |
| `itemSearchHotkey` | `string` | `''` | ウィンドウ検索の起動ホットキー（空=無効） |
...
| `parallelGroupLaunch` | `boolean` | `false` | グループアイテムを並列起動 |
...
| `backupRetention` | `number` | `10` | バックアップ保存件数（範囲: 3-50） |
| `backupIncludeClipboard` | `boolean` | `false` | クリップボードデータをバックアップに含める |
| `workspacePositionMode` | `string` | `'displayRight'` | ワークスペース表示位置モード（displayLeft/displayRight/fixed） |
| `workspaceTargetDisplayIndex` | `number` | `0` | ワークスペースを配置するディスプレイ番号（0始まり） |
| `workspaceVisibleOnAllDesktops` | `boolean` | `true` | ワークスペースを全仮想デスクトップに表示 |
...
### 7.5.1. ウィンドウ検索の起動ホットキー

ウィンドウ検索モードで起動するためのオプションのホットキー。
...
**無効化:**
- 入力欄をクリアすると機能が無効になります（「×」ボタンでクリア可能）

**動作:**
- このホットキーで起動すると、自動的にウィンドウ検索モードが有効になります
- 通常のホットキー（Alt+Space等）とは別に設定可能
...
| `backupRetention` | `10` | 保存件数上限（3-50件） |
| `backupIncludeClipboard` | `false` | クリップボードデータをバックアップに含める |

**保存場所:** `%APPDATA%/quick-dash-launcher/config/backup/`

**ファイル名:** `data_2024-01-01T12-00-00.json`
