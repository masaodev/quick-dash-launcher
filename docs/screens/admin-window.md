...
2. 入力フィールドからフォーカスが外れたとき（onBlur）にタブ情報を更新
3. **即座保存**: 設定が自動的に保存され、メインウィンドウに反映される
4. メインウィンドウとアイテム管理画面のタブ名/ドロップダウンがリアルタイムに更新される

**注意**: タブ名の保存は、入力フィールドからフォーカスが外れたときに自動的に実行されます（v1.0.0以降）。明示的な保存ボタンのクリックは不要です。
...
- **関連ファイル一覧**: タブに含まれるファイルのリストを表示
  - **データファイル名入力欄**: 各ファイルに任意のデータファイル名（別名）を設定可能
    - プレースホルダー: 「データファイル名（必須）」
    - 入力後、フォーカスが外れると自動保存
    - データファイル名はグローバルで、全てのタブで共通
    - 空の場合はエラーアラートが表示される（必須項目）
  - **物理ファイル名**: 括弧内に物理ファイル名を表示（例: (data.txt)）
...
**データファイル名の表示:**
- タブバー: データファイル名が設定されている場合、ツールチップに「データファイル名 (物理ファイル名)」と表示
- 編集モード: ファイル選択ドロップダウンでデータファイル名が優先表示され、ツールチップに物理ファイル名が表示
...
#### 処理フロー
1. 確認ダイアログを表示：「グループ「{groupName}」をアーカイブから完全に削除しますか？」
2. OKの場合：
