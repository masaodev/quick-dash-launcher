...
- **タブ名表示形式（v1.0.0以降）**:
  ...
  - カスタムタブ名がない場合: デフォルトタブ名を表示（例: `メイン`、`サブ1`、`サブ2`）
  - ファイル名は省略されます（「タブ: メイン」「データファイル: data.txt」のように分離表示）
...
- **CSVエスケープ処理**: アイテムの登録・編集時、ダブルクォートやカンマを含む引数を自動的にエスケープ処理（詳細は[CSV形式のエスケープルール](../architecture/data-format.md#csv形式のエスケープルール)を参照）
- 行の追加、削除、内容編集が可能
...
- **単一アイテム選択時**: 単一アイテム登録（名前・パス・引数の編集が可能）
  - **引数フィールド**: ダブルクォートやカンマを含む引数も、通常の文字列として入力するだけで自動的にエスケープされます
  - 例: `-p "Git Bash" -d "C:\path"` と入力すると、保存時に `"-p ""Git Bash"" -d ""C:\path"""` として自動エスケープされます
...
##### CSVエスケープの自動処理
引数フィールドに入力された内容は、保存時に自動的にCSVエスケープ処理が行われます。ユーザーはエスケープ処理を意識する必要はありません。

**自動エスケープ処理の例**:
- **入力**: `-p "Git Bash" -d "C:\Projects"`
- **保存**: `"-p ""Git Bash"" -d ""C:\Projects"""`（ファイル内）
- **表示**: `-p "Git Bash" -d "C:\Projects"`（編集画面）
...
- **対象範囲**: 現在選択中のデータファイルのみ
  - 例: data.txtを選択している場合、data.txtのデータのみが対象
  - 他のデータファイル（data2.txt等）には影響しません
- **整列順序**: 種類 → パスと引数 → 名前
