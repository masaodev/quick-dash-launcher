...
- **グローバルホットキー**: Alt+Spaceで即座にランチャーを呼び出し（設定画面でカスタマイズ可能）
- **ウィンドウ固定化**: 📌ボタンでウィンドウを固定し、フォーカスアウトしても非表示にならない
- **アイコン表示**: アプリケーション、ファイル、URLのアイコンを自動取得・表示
- **タブ切り替え**: 複数のデータファイル（data.txt, data2.txt...）をタブで切り替え、Tab/Shift+Tabキーで操作可能
...
アプリケーションを初めて起動すると、初回設定画面が自動的に表示されます。
...
1. **ホットキー入力欄をクリック**して、希望するキー組み合わせを実際に押下
   - 例: `Ctrl+Alt+Q`、`Ctrl+Shift+L`など
   - リアルタイムでバリデーションが実行され、使用可能かチェックされます
   - デフォルトは `Alt+Space` が入力されています

2. **設定を完了**
   - 「設定を完了」ボタンをクリックして設定を保存

設定完了後、メインウィンドウが表示され、アプリケーションが利用可能になります。

#### 初回設定画面の特徴

- **スプラッシュ画面終了後に自動表示**: アプリケーション起動時、スプラッシュウィンドウの後に自動的に表示されます
- **閉じることができない**: 初回設定が完了するまで、ESCキーやフォーカス喪失では閉じません
- **ホットキーが空の場合に表示**: 設定ファイルの`hotkey`項目が空文字列または未設定の場合に初回設定画面が表示されます

### 初回設定のスキップ（開発者向け）

開発中にこの画面をスキップしたい場合は、設定ファイル（`%APPDATA%/quick-dash-launcher/config/settings.json`）を直接編集して`hotkey`に任意の値（例: `"Alt+Space"`）を設定してください。
