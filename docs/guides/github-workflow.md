...
#### 自動品質チェック（推奨）
Claude Codeの`quality-checker`サブエージェントを使用すると、総合的な品質チェックを実行できます：

```
Task tool with subagent_type="quality-checker"
```

このサブエージェントは以下を自動実行します：
...
4. **品質チェック**: コミット・プッシュ前に`quality-checker`サブエージェントを実行（推奨）
5. **PR作成**: テンプレートに従って記載
6. **レビュー**: フィードバックに基づいて修正
7. **マージ**: mainブランチへマージ
