...
1. **初回起動時のホットキー設定画面**
   - アプリケーション初回起動時に自動表示
   - グローバルホットキーの設定（デフォルト: Ctrl+Alt+W）
   - リアルタイムバリデーション
...
2. **グローバルホットキー（Ctrl+Alt+W）** - 登録とウィンドウトグル
...
7. **ファビコン一括取得** - 🌐ボタンでウェブサイトのファビコン取得
8. **アプリケーションアイコン一括抽出** - 🎨ボタン（URLを除く）
9. **カスタムURIアイコン一括抽出** - 🎨ボタン
